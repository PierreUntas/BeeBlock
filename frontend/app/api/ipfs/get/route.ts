...
const rawCustomGateway = process.env.IPFS_GATEWAY_URL ?? '';

// Normalise pour s'assurer d'avoir la forme `.../ipfs/`
function normalizeGateway(url: string): string {
    if (!url) return '';
    let u = url.trim();
    // retirer éventuels suffixes redondants
    u = u.replace(/\/?ipfs\/?$/i, '');
    // ajouter le segment /ipfs/ à la fin
    if (!u.endsWith('/')) u += '/';
    return `${u}ipfs/`;
}

const customGateway = normalizeGateway(rawCustomGateway);

// Construire la liste finale en mettant le gateway custom en tête et en évitant les doublons
